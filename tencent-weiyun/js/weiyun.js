...
addFileOverOut();//作用：给fileBox里的所有newBox添加移入移出事件

function addFileOverOut(){
...
function fullTip(classNames,message,removeName){
    //先瞬间拉回到-32，在运动到0
    fullTipBox.style.transition = "none";
    fullTipBox.style.top = "-32px";
    
    setTimeout(function (){
    	var arr = removeName.split(" ");
    	for (var i = 0; i < arr.length; i++) {
    		tools.removeClass(fullTipBox,arr[i]);   		
    	}
...
//点击newBox 文件区域(给每个newBox添加点击处理函数，委托给fileBox)
tools.addEvent(fileBox,"click",function(ev){
	...
	if(tools.parents(target,".newBox")){
		...
		if(tools.hasClass(target,"icoBox") ){
			
			var hadClick = tools.toggleClass(target,"icoBoxChecked");//返回true说明之前没有，现在用toggle加上了
...
tools.addEvent(menu,"click",function(ev){
	var target = ev.target;
	if(tools.hasClass(target,"ico")){//如果点击三角形
		
			var tree = target.parentNode.parentNode;//获取div
	...
	}else if(tools.parents(target,".tree_title") ){//如果点击的是div
...
//点击新建按钮的时候，click包括down和up两个过程，此时down会冒泡到doc上，所以会先document的down事件，doc.down的作用是：新建你完成，点击doc时，判断新建文件名称是否为空，是否重名，以上两种情况都不成立，说明新建成功，此时要把数据push到dataList中，且开关改成false
	...
	var inputText =tools.$(".inputTitle",firstChild)[0];//input
		...
		if(ev.keyCode == 13){
			...
			if(val === ""){//为空
				...
				if( handleData.reNameWithoutSelf(dataList,parentId,val,newBoxId) ){//重名
					...
					fullTip("warn","文件夹名有重复，请重新命名","err warn ok");
				}else{//成功
		            ...
		            fullTip("ok","新建文件夹成功","err warn ok");	//提醒
	...
	remindBlankFile();//不管新建是否成功，都要添加空白文件提醒，成功了remove，不成功add
})
...
tools.addEvent(document,"mousedown",function(){
	if(create.isCreate){    
		...
		if(inputValue === ""){
			...
			fullTip("warn","文件夹名有冲突，请重新命名","err warn ok");
...
//-------------------------------------------------------------------------------------------	
	...
	if(rechristen.isClick){
		...
		var inputText = tools.$(".inputTitle",newBox)[0];
		...
		for (var i = 0; i < selectArr.length; i++) {
			...
			for (;j<children.length;j++){//注意j拿出去的话，for()中要加个;
				if(dragId.title == children[j].title ){
					//提示：文件名重复，移入失败
					fullTip("warn","部分移动失败,重名了","err warn ok");
					break;
				}
			}
			...
			//通过dragId找到被移动的div
			var moveDiv = tools.getTreeById("tree_title",selectArr[i].dataset.fileId,menu);//div
			//moveDiv中有很多子孙节点，调用下面的函数，给moveDiv以及它所有的子孙节点都设置好padding-left
			handleData.setDivPaddingLeft(moveDiv);
			
			
			var moveLi = moveDiv.parentNode;//li
			var moveUl = moveDiv.parentNode.parentNode;//ul
			moveUl.removeChild(moveLi);
			
			var passiveDiv = tools.getTreeById("tree_title",passiveId,menu);//div
			tools.addClass(passiveDiv,"showIco");
			tools.removeClass(passiveDiv,"noIco");
			var passiveNext = passiveDiv.nextElementSibling;//ul
			
			
			passiveNext.appendChild(moveLi);
			passiveNext.style.display = "block";
...
tools.addEvent(rechristen,"click",function(ev){
	...
	if(selectArr.length === 0){
		 fullTip("warn","请选择文件","err warn ok");	//提醒;
	}else if(selectArr.length > 1){
		 fullTip("err","只能针对一个文件夹重命名","err warn ok");	//提醒;
	}else{//重命名成功
			...
			if(ev.keyCode == 13){
				...
				if(val === ""){
					...
					if( handleData.reNameWithoutSelf(dataList,parentId,val,newBoxId) ){
						...
						handleData.getDataById(dataList,newBoxId).title = val;
...
tools.addEvent(deleteSome,"click",function(){
	...
	if(selectArr.length === 0){
		fullTip("warn","请选择文件","err warn ok");
	}else{
...
tools.addEvent(moveTo,"click",function(ev){
	...
	if(selectArr.length === 0){
				...
				if(!canMove){
					...
					for (var i = 0; i < selectArr.length; i++){
						...
						for(var j = 0; j<children.length; j++){
							if(curData.title ==children[j].title ){
								fullTip("warn","(1051),文件名重复","err warn ok");
								onOff = false;
								break;
							}
						}
					...
					for (var i = 0; i < selectArr.length; i++) {
						...
						if(onOff){//如果为true，此时可以移动了
							...
							//被移动元素的pid改成弹框中间菜单区域点击的那个div的id
							curData.pid = moveId;
							...
							//从父级中删除被移动的元素
							var Movediv = tools.getTreeById("tree_title",curData.id,menu);//被移动的那个元素div
							var divParent = Movediv.parentNode;//被移动元素的父级(li)
							
							
							var divTarget = tools.getTreeById("tree_title",moveId,menu);//被移入的文件div
							var nextUl = divTarget.nextElementSibling;//被移入文件li中的ul
							
							nextUl.appendChild(divParent);//被移动的li移入到当前的ul中
							
							//给movediv以及设置padding-left(根据层级)
							Movediv.style.paddingLeft = handleData.getLevel(dataList,curData.id)*14 + "px";
		...
		//点击移动时弹框中的数菜单中每个div(tree)时，利用事件委托给它们的父级dirTree
		tools.addEvent(dirTree,"click",function(ev){
			var target = ev.target;
			if( tools.parents(target,".tree_title") ){//如果当前点击的是div
				
				target = tools.parents(target,".tree_title");//target由原来的小目标源变成当前的div
				
				canMove = false;//false代表可以移动,此时执行(行967)中的代码
